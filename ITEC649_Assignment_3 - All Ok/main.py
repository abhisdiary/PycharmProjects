...
def category(db, cat):
    ...
    if cat is not None:
        ...
        if len(products['product_list']) == 0:
            products['title'] = "No Product in this Category"
        else:
...
def product(db, id):
    ...
    if len(product_individual['individual_product']) == 0:
        product_individual['title'] = "No Such Product"
    else:
        product_individual['title'] = "The WT"
...
resulting page will contain the updated shopping cart contents."""
    session.get_or_create_session(db)
    product_id = request.forms.get('item')
