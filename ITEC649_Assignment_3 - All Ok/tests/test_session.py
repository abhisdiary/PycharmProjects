...
class SessionTests(unittest.TestCase):
    ...
    def test_get_or_create_session(self):  # passed
    ...
    def test_session_bad_cookie(self):  # passed
    ...
    def test_cart_update(self):  # Not passed.....see below
        ...
        # quantity is doubled
        session.add_to_cart(self.db, product['id'], quantity)  # -----> extra parameter: update=False
        ...
        self.assertEqual(str(quantity * 2), cart[0]['quantity'])  # ------>  quantity needs to be string

        # now add again but with the update flag set, this should
        # set the quantity rather than adding to it
        session.add_to_cart(self.db, product['id'], quantity)  # -----> extra parameter: update=True
        # session.add_to_cart(self.db, product['id'], quantity, update=True)
        cart = session.get_cart_contents(self.db)

        self.assertEqual(1, len(cart))
        self.assertEqual(product['id'], cart[0]['id'])
        self.assertEqual(quantity, cart[0]['quantity'])  # 9!=3 means what? We are not told about the update flag in the readme file
