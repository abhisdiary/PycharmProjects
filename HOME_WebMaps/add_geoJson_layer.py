...
for lt, ln, el, name in zip(lat, lon, elev, name):
    ...
    m.add_child(folium.CircleMarker(location=[lt, ln], radius=6, popup=folium.Popup(iframe, parse_html=True), fill_color=color_producer(el), color='grey',
                                    fill=True, fill_opacity=0.7))

# 3rd Layer
fg.add_child(folium.GeoJson(data=(open("res/world.json", "r", encoding="utf-8-sig").read()),
                            style_function=lambda x: {
                                'fillColor': 'green' if x['properties']['POP2005'] < 1000000 else 'orange' if 1000000 <= x['properties'][
                                    'POP2005'] < 2000000 else 'red'}))

m.add_child(fg)
m.save("views/GeoJson_Layer.html")
